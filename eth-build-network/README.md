...
* `create-accounts.sh`: primitive wrapper to [geth](https://github.com/ethereum/go-ethereum)
* `eth-build-network.sh`: launch local clusters non-interactively (https://github.com/ethereum/go-ethereum/wiki/Setting-up-private-network-or-local-cluster)
...
```
GETH=./geth bash /path/to/eth-utils/create-accounts.sh <rootdir> <dd> <run> <params>...
```
...
```
$ GETH=./geth bash ~/eth-utils/create-accounts.sh ~/tmp/eth/ 04 09 --mine console 
...
```
GETH=./geth bash eth-build-network.sh <root> <n> <network_id> <runid> <IP> [[params]...]
```
...
- the nodes can be restarted from the same state individually using the `create-accounts.sh` script
...
- you can supply additional params on the command line which will be passed 
  to `create-accounts.sh` and eventually to `geth` for each node, for instance `-vmodule=http=6 -mine -minerthreads=8` is a good one.

```
GETH=./geth bash eth-build-network.sh ./leagues/3301/cicada 2 3301 05 77.160.58.3 -mine 
